  ...
  return (
    <div>
      <h2 id="product-heading" data-cy="ProductHeading">
        <Translate contentKey="dailiesApp.product.home.title">Products</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="dailiesApp.product.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/product/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="dailiesApp.product.home.createLabel">Create new Product</Translate>
          </Link>
        </div>
      </h2>
        ...
        {productList && productList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="dailiesApp.product.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('stockQty')}>
                  <Translate contentKey="dailiesApp.product.stockQty">Stock Qty</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('stockQty')} />
                </th>
                ...
                <th className="hand" onClick={sort('tax')}>
                  <Translate contentKey="dailiesApp.product.tax">Tax</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('tax')} />
                </th>
                <th className="hand" onClick={sort('mrp')}>
                  <Translate contentKey="dailiesApp.product.mrp">Mrp</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('mrp')} />
                </th>
                <th className="hand" onClick={sort('offerText')}>
                  <Translate contentKey="dailiesApp.product.offerText">Offer Text</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('offerText')} />
                </th>
                <th className="hand" onClick={sort('description')}>
                  <Translate contentKey="dailiesApp.product.description">Description</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('description')} />
                </th>
                <th className="hand" onClick={sort('disclaimer')}>
                  <Translate contentKey="dailiesApp.product.disclaimer">Disclaimer</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('disclaimer')} />
                </th>
                <th className="hand" onClick={sort('subscription')}>
                  <Translate contentKey="dailiesApp.product.subscription">Subscription</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('subscription')} />
                </th>
                ...
                <th>
                  <Translate contentKey="dailiesApp.product.subCat">Sub Cat</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {productList.map((product, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{product.subscription ? 'true' : 'false'}</td>
                  ...
                  <td>{product.subCat ? <Link to={`/sub-cat/${product.subCat.id}`}>{product.subCat.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="dailiesApp.product.home.notFound">No Products found</Translate>
            </div>
          )
        )}
      </div>
