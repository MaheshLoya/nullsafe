...
public interface OrdersRepository extends JpaRepository<Orders, Long>, JpaSpecificationExecutor<Orders> {
    ...
    @Query(value = "select orders from Orders orders left join fetch orders.user", countQuery = "select count(orders) from Orders orders")
    Page<Orders> findAllWithToOneRelationships(Pageable pageable);

    @Query("select orders from Orders orders left join fetch orders.user")
    List<Orders> findAllWithToOneRelationships();

    @Query("select orders from Orders orders left join fetch orders.user where orders.id =:id")
